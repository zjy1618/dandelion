...
function photoClip(container, option) {
	var option = option || {};
	var clipWidth = option.width || 256,
		clipHeight = option.height || 256,
		file = option.file || '',
		outputType = option.outputType || "jpg",
		strictSize = false,
		loadStart = option.loadStart || function() {},
		loadComplete = option.loadComplete || function() {},
		loadError = option.loadError || function() {};
	if(option.hasOwnProperty('strictSize')) strictSize = opt.strictSize;
	...
	function init() {
		...
		// 创建裁剪视图层
		$clipView = $("<div class='photo-clip-view slides'>").css({
			"position": "absolute",
			"left": "50%",
			"top": "50%",
			"width": clipWidth,
